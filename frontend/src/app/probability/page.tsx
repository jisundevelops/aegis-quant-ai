...
export default function ProbabilityPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Scenario Probabilities</h1>
        <p className="text-sm text-[var(--fg-muted)] mt-1">
          Bull / Bear / Range probabilities derived from agent votes. Always sum
          to 100%.
        </p>
      </div>
      ...
      <div className="bg-bg-card border border-border rounded-lg p-4">
        <div className="flex flex-wrap items-end gap-4">
          ...
          >
            {loading ? "Computing..." : "Compute Probabilities"}
          </button>
        </div>
        {error && (
      ...
      {!signal && !loading && (
        <div className="bg-bg-card border border-border rounded-lg p-12 text-center">
          <p className="text-[var(--fg-muted)]">
            Run an analysis to see scenario probabilities.
          </p>
        </div>
      )}
    </div>
